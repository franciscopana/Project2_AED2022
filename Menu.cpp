...
vector<string> Menu::getAirportsCode() {
    while (true) {
        ...
        //Airports code
        if(input.size()==3 && all_of(input.begin(), input.end(), ::isalpha)){
        ...
        result = database.getAirportsCodeFromCountry(input);
        if(result.empty()){
        ...
        if(!input.empty()) {
            cout << "It must either be the name of a city, a country, a set of airports code or a pair of coordinates. Try again: ";
        }
    }
}